...
class ConvBlock(nn.Module):
    """
    A simple 3 x 3 convolution block with padding size 1, followed by a ReLU.
    """
...
class CNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): a tensor with shape `(B, Ch, H, W)`.

        Returns:
            torch.Tensor: a tensor with shape `(B, Cl)`.s
        """
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument("--fc_dropout", type = int, default = FC_DROPOUT)
        return parser